...
async function generateAutofillScriptWithLLM(html: string): Promise<string> {
  ...
  const prompt = `You are a browser automation expert. Given the following HTML of a job application page, generate a JavaScript snippet that fills in all visible form fields (text, email, textarea, select, radio, checkbox) with realistic dummy data. For each field, always check if the element exists before setting its value or properties (e.g., if (el) { el.value = ... }). Do not submit the form or click any final submit buttons. Only fill the fields. Do NOT use document.addEventListener('DOMContentLoaded', ...) or any similar wrappers; assume the script runs immediately. Output only the JavaScript code, no explanations.\n\nHTML:\n${html}`;
